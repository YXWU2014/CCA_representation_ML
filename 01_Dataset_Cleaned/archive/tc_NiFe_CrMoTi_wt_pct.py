...
with TCPython() as start:
    ...
    k = 40
    list_of_conditions = [(("W(Ni)", w_ni), ("W(Cr)", 0.1E-2), ("W(Mo)", 0.1E-2), ("W(Ti)", 0.1E-2), ("T", tk))
                          for w_ni in np.linspace(0.1E-2, 0.95, k)
                          for tk in np.arange(900+273.15, 1300+273.15, 50)
                          if w_ni + 0.1E-2 + 0.1E-2 + 0.1E-2 <= 1]
...
# Plotting
plt.scatter(result_FCC_y['Ni'], len(result_FCC_y['Ni'])
            * [1], marker='o', color='blue', label='Y values')
plt.scatter(result_FCC_n['Ni'], len(result_FCC_n['Ni'])
            * [1], marker='o', color='red', label='N values')

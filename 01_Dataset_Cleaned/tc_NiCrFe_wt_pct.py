...
with TCPython() as start:
    ...
    list_of_conditions = [(("W(Ni)", w_ni), ("W(Cr)", w_cr), ("T", tk))
                          for w_ni in np.linspace(0.0, 0.95, k)
                          for w_cr in np.linspace(0.0, 0.95, k)
...
df_output_fname = "tc_NiCrFe_wt_pct"
...
result_FCC_y.to_excel(df_output_fname + ".xlsx", index=False)
...
plt.ylim(0, 100)
plt.axis('square')
plt.savefig(df_output_fname + '.png')

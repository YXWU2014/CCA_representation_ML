...
class ModelExplainer:
    ...
    def _predict_for_one_fold(self, X_list, Y_list, V_list, i):
        try:
            ...
            print(f"Exception in _predict_for_one_fold for fold {i}: {e}")
            raise

    def _shap_for_one_fold_oneRepeat(self,
        ...
        try:
            ...
            model = self._load_model(i)
            input_base_data = self._prepare_input_data(
                X_base_list, V_base_list, i)
            input_shap_data = self._prepare_input_data(
                X_shap_list, V_shap_list, i)
            ...
            if Y_shap_list[i].size != 0:
                ...
                return i, np.concatenate([shap_values_oneFold[0][0], shap_values_oneFold[0][1]], axis=1)
            else:
                # print('SHAP shape : ', shap_values_oneFold[0].shape)
                return i, shap_values_oneFold[0]

        except Exception as e:
            print(
                f"Exception in _shap_for_one_fold_oneRepeat for fold {i}: {e}")
            raise
    ...
    def predict_shap_bootstrap_norm(self,
        ...
        # run predictions for base and shap data
        ...
        for i in range(self.k_folds * self.n_CVrepeats):
            for mc_index in range(self.mc_repeat):
                future = delayed(self._shap_for_one_fold_oneRepeat)(X1_base_list, Y1_base_list, V1_base_list,
                                                                    X1_shap_list, Y1_shap_list, V1_shap_list,
                                                                    i)
                future_shap_values.append(future)

        # Calculate 80% of available CPUs
        total_cpus = multiprocessing.cpu_count()
        cpus_to_use = int(total_cpus * 0.4)  # Using 80% of CPUs, rounded down
        shap_values_results = Parallel(n_jobs=cpus_to_use)(future_shap_values)

        # # Organize SHAP values by fold based on the returned fold index
        shap_values_by_fold = {i: []
                               for i in range(self.k_folds * self.n_CVrepeats)}

        for fold_index, shap_values in shap_values_results:
            shap_values_by_fold[fold_index].append(shap_values)

        averaged_shap_values_by_fold = [np.mean(np.array(shap_values_fold), axis=0)
                                        for shap_values_fold in shap_values_by_fold.values()]

        shap_list = averaged_shap_values_by_fold
...
def inverse_norm_shap(pred_norm_base_stack,
    ...
    # print(np.abs(pred_norm_base_mean_AVG + shap_norm_mean.sum(axis=1) - pred_norm_shap_mean))
